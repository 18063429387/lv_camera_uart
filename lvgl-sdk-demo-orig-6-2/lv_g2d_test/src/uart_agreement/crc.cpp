...
unsigned short CRC16( unsigned char * pucFrame, unsigned int usLen )
{
    ...
    return ( unsigned short )( ucCRCHi << 8 | ucCRCLo );     /*!< here no think about endian mode */
}

/*********************************************************************************************************
** Function name:       crc7Update
** Descriptions:        ????CRC7
** input parameters:    ucOri: ???CRC??????????pucData?§Ö?CRC????????????0
**                      pucData: ???????CRC???????????
**                      uiLen: ???????CRC???????
** output parameters:   ????????CRC7???
** Returned value:      ??????
*********************************************************************************************************/
...
??  ????????????Crc16?????????????
*********************************************/
unsigned short mbGetCRC16(unsigned short crcInit /* ???? 0 */, unsigned char *ptr, unsigned int len) 
{
