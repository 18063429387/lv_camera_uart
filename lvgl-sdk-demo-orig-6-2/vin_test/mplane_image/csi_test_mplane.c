...
static int disp_set_addr(int width, int height, struct v4l2_buffer *buf)
{
	...
	/* 优先使用 DISP_LAYER_SET_CONFIG2 + dmabuf fd（更符合新内核 display2 接口） */
	if (use_disp_config2 && capture_mode == 4 && nplanes == 1) {
		disp_layer_config2 cfg2;

		memset(&cfg2, 0, sizeof(cfg2));
		cfg2.channel = dev.layer_info.layer_config.channel;
		cfg2.layer_id = dev.layer_info.layer_config.layer_id;
		cfg2.enable = 1;
		cfg2.info.mode = LAYER_MODE_BUFFER;
		cfg2.info.zorder = dev.layer_info.layer_config.info.zorder;
		cfg2.info.alpha_mode = dev.layer_info.layer_config.info.alpha_mode;
		cfg2.info.alpha_value = dev.layer_info.layer_config.info.alpha_value;

		cfg2.info.screen_win = dev.layer_info.layer_config.info.screen_win;

		cfg2.info.fb.fd = buffers[buf->index].dmabuf_fd[0];
		cfg2.info.fb.trd_right_fd = -1;
		cfg2.info.fb.format = DISP_FORMAT_YUV420_SP_UVUV;
		cfg2.info.fb.color_space = DISP_BT709;

		cfg2.info.fb.size[0].width = width;
		cfg2.info.fb.size[0].height = height;
		cfg2.info.fb.size[1].width = width / 2;
		cfg2.info.fb.size[1].height = height / 2;

		/* 32.32 定点格式 */
		cfg2.info.fb.crop.x = 0;
		cfg2.info.fb.crop.y = 0;
		cfg2.info.fb.crop.width = (unsigned long long)width << 32;
		cfg2.info.fb.crop.height = (unsigned long long)height << 32;

		arg[0] = dev.layer_info.screen_id;
		arg[1] = (unsigned long)&cfg2;
		arg[2] = 1;
		arg[3] = 0;
		ret = ioctl(dev.layer_info.dispfh, DISP_LAYER_SET_CONFIG2, (void *)arg);
		if (ret != 0)
			printf("disp_set_addr: DISP_LAYER_SET_CONFIG2 failed\n");
	...
	dev.layer_info.layer_config.info.fb.crop.x = 0;
	dev.layer_info.layer_config.info.fb.crop.y = 0;
	...
	if (nplanes == 1) {
		/* 单平面：当前主要支持 NV12（mode == 4） */
		unsigned long long y_size = (unsigned long long)width * height;

		dev.layer_info.layer_config.info.fb.addr[0] =
		    buf->m.planes[0].m.mem_offset;

		if (capture_mode == 4) {
			/* NV12: Y 在 addr[0]，UV 紧接其后，DE 需要显式 UV 基址 */
			dev.layer_info.layer_config.info.fb.addr[1] =
			    dev.layer_info.layer_config.info.fb.addr[0] + y_size;
			dev.layer_info.layer_config.info.fb.addr[2] = 0;
			dev.layer_info.layer_config.info.fb.size[1].width = width / 2;
			dev.layer_info.layer_config.info.fb.size[1].height = height / 2;
		} else if (dev.layer_info.pixformat == TVD_PL_YUV420) {
			dev.layer_info.layer_config.info.fb.addr[1] =
			    dev.layer_info.layer_config.info.fb.addr[0] + y_size;
			...
			dev.layer_info.layer_config.info.fb.size[1].width =
			    width / 2;
			dev.layer_info.layer_config.info.fb.size[1].height =
			    height / 2;
			dev.layer_info.layer_config.info.fb.size[2].width =
			    width / 2;
			dev.layer_info.layer_config.info.fb.size[2].height =
			    height / 2;
		} else {
			/* 其它单平面格式，保守只用 addr[0] */
			dev.layer_info.layer_config.info.fb.addr[1] = 0;
			dev.layer_info.layer_config.info.fb.addr[2] = 0;
		}
	} else {
		/* 多平面情况，保持原有逻辑，但增加下标保护 */
		if (dev.layer_info.pixformat == TVD_PL_YUV420) {
			/* YUV420 planar，3 plane */
			dev.layer_info.layer_config.info.fb.size[1].width =
			    width / 2;
			dev.layer_info.layer_config.info.fb.size[1].height =
			    height / 2;
...
static int disp_disable(void)
{
	...
	/*close channel 4 (video overlay used by this program) */
	memset(&disp, 0, sizeof(disp_layer_config));
	disp.channel = 4;
	disp.layer_id = 0;
	disp.enable = 0;
	...
	ret = ioctl(dev.layer_info.dispfh, DISP_LAYER_SET_CONFIG, (void *)arg);
	if (ret != 0)
		printf("disp_disable: channel 4 fail to set layer info\n");

	/* 同时尝试用 config2 关闭（若本程序走的是 SET_CONFIG2） */
	if (use_disp_config2) {
		memset(&disp2, 0, sizeof(disp_layer_config2));
		disp2.channel = 4;
		disp2.layer_id = 0;
		disp2.enable = 0;
		arg[0] = dev.layer_info.screen_id;
		arg[1] = (unsigned long)&disp2;
		arg[2] = 1;
		arg[3] = 0;
		ioctl(dev.layer_info.dispfh, DISP_LAYER_SET_CONFIG2, (void *)arg);
	}
...
static int disp_init(int width, int height, unsigned int pixformat)
{
	...
	/* 尝试关闭 HDMI overscan/underscan 影响，避免右侧被裁剪 */
	// {
	// 	unsigned long arg[6];
	...
	// 	arg[2] = 0;
	// 	arg[3] = 0;
	// 	if (ioctl(dev.layer_info.dispfh, DISP_DEVICE_GET_CONFIG, (void *)arg) == 0) {
	// 		cfg.scan = UNDERSCAN;
	// 		if (ioctl(dev.layer_info.dispfh, DISP_DEVICE_SET_CONFIG, (void *)arg) != 0)
	// 			printf("DISP_DEVICE_SET_CONFIG failed\n");
	// 	} else {
	// 		printf("DISP_DEVICE_GET_CONFIG failed\n");
	// 	}
	// }

	/*
	 * 使用实际屏幕尺寸（如 RGB 1024x600），否则 layer 会按 1920x1080 超出屏外。
	 * 从 /dev/fb0 读取当前显示分辨率；失败则用采集分辨率。
	 */
	dev.layer_info.width = width;
	dev.layer_info.height = height;
	fbfd = open("/dev/fb0", O_RDONLY);
	if (fbfd >= 0) {
		CLEAR(var);
		if (ioctl(fbfd, FBIOGET_VSCREENINFO, &var) == 0) {
			dev.layer_info.width = var.xres;
			dev.layer_info.height = var.yres;
			printf("display size from fb0: %u x %u\n",
			       dev.layer_info.width, dev.layer_info.height);
		}
		close(fbfd);
	}

	dev.layer_info.output_type = DISP_OUTPUT_TYPE_LCD;

	dev.layer_info.pixformat = pixformat;
	/* 使用 channel 0 / layer 0 作为视频层 */
	...
	/*
	 * 根据采集模式选择合适的显示像素格式：
	 * mode == 4: V4L2_PIX_FMT_NV12 → DISP_FORMAT_YUV420_SP_UVUV
	 * 其它：沿用原先 YUV420 planar / YUV422 SP 配置
	 */
	if (capture_mode == 4) {
	...
	/* 将屏幕宽度减少6倍的margin值 （改为不减少）*/
	{
		struct disp_screen screen;
		int margin = 100; /* 左右各缩 100 像素 */
		screen = get_disp_screen(dev.layer_info.width,
						 dev.layer_info.height,
						 width,
						 height);
		// if (screen.w > (unsigned int)(margin * 2)) {
		// 	// screen.x += margin;
		// 	screen.w -= margin * 6;
		// }
		dev.layer_info.layer_config.info.screen_win.x = screen.x;
		dev.layer_info.layer_config.info.screen_win.y = screen.y;
		dev.layer_info.layer_config.info.screen_win.width = screen.w;
		dev.layer_info.layer_config.info.screen_win.height = screen.h;
	}
#endif
	return 0;
}
...
static int req_frame_buffers(void)
{
	...
	for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
		...
		for (i = 0; i < nplanes; i++) {
			...
			buffers[n_buffers].dmabuf_fd[i] = -1;
			...
			if (-1 == ioctl(fd, VIDIOC_EXPBUF, &exp)) {
				/*
				 * 某些驱动不支持 EXPBUF，送显将回退到旧的 SET_CONFIG
				 * （但可能仍然无法显示）。
				 */
				/* printf("VIDIOC_EXPBUF error\n"); */
				buffers[n_buffers].dmabuf_fd[i] = -1;
			} else {
				buffers[n_buffers].dmabuf_fd[i] = exp.fd;
			}
		}
		free(buf.m.planes);
	}
	...
	/*
	 * 暂不使用 DISP_LAYER_SET_CONFIG2：当前内核的 disp_layer_config2 与
	 * 用户态头文件布局不一致，会触发 disp_get_layer(0,63,79696) 并内核 Oops。
	 * 送显仅走 DISP_LAYER_SET_CONFIG + mem_offset 路径。
	 */
	use_disp_config2 = 0;
...
static int free_frame_buffers(void)
{
	...
	for (i = 0; i < n_buffers; ++i) {
		for (j = 0; j < nplanes; j++) {
			...
			if (buffers[i].dmabuf_fd[j] >= 0) {
				close(buffers[i].dmabuf_fd[j]);
				buffers[i].dmabuf_fd[j] = -1;
			}
		}
	}
	free(buffers);
...
static int camera_fmt_set(int mode)
{
	...
	if (-1 == ioctl(fd, VIDIOC_G_FMT, &fmt)) {
		...
		bytesperline[0] = fmt.fmt.pix_mp.plane_fmt[0].bytesperline;
		bytesperline[1] = fmt.fmt.pix_mp.plane_fmt[1].bytesperline;
		bytesperline[2] = fmt.fmt.pix_mp.plane_fmt[2].bytesperline;
		printf("resolution got from sensor = %d*%d num_planes = %d bytesperline = [%u %u %u]\n",
		       fmt.fmt.pix_mp.width, fmt.fmt.pix_mp.height,
		       fmt.fmt.pix_mp.num_planes,
		       bytesperline[0], bytesperline[1], bytesperline[2]);
	}
